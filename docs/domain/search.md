...
### `useStorePageController` 카테고리·키워드 연동
...
| `?keyword=` | 검색 키워드 | `?keyword=오독오독` |

- `categoryId` 또는 `keyword` 변경 시 `setPage(1)` dispatch로 페이지 초기화
- `keyword` 존재 시 `category` 파라미터를 `undefined`로 설정 — 키워드 단독 전체 검색
- `keyword` 없을 때만 `categoryId !== 'ALL'`이면 `category` 파라미터 전달
...
## SearchBar 컴포넌트 (`src/features/components/layout/SearchBar.jsx`)

Header에 상시 표시되는 검색창. 실서버 API 연동 완료 (2026-04-29).

### 동작 흐름

| 상태 | 드롭다운 내용 |
|---|---|
| 포커스 + 입력 없음 | 인기검색어 태그 목록 (`useGetTrendingKeywordsQuery`) |
| 포커스 + 입력 있음 + 자동완성 결과 있음 | 자동완성 목록 (`useLazyGetAutocompleteQuery`, 최대 8개) |
| 포커스 + 입력 있음 + 자동완성 결과 없음 | 드롭다운 미표시 |
| 포커스 해제 | 드롭다운 닫힘 |

### 자동완성 디바운스

입력 후 **300ms** 대기 후 `triggerAutocomplete(keyword)` 호출. 빠른 타이핑 시 불필요한 API 요청 방지.

### 키보드 네비게이션

`document.addEventListener('keydown')` 방식 — input DOM focus 여부와 무관하게 동작.

| 키 | 동작 |
|---|---|
| `ArrowDown` | 다음 항목 하이라이트 |
| `ArrowUp` | 이전 항목 하이라이트 (첫 항목에서 → 인풋으로 복귀) |
| `Enter` | 하이라이트 항목 검색 (없으면 입력값으로 검색) |
| `Escape` | 드롭다운 닫기 |

마우스 호버와 키보드 네비게이션은 동일한 `highlightedIndex` 상태를 공유.
...
- 인기검색어 태그 클릭 → 즉시 검색 결과 이동
- 자동완성 항목 클릭 → 즉시 검색 결과 이동
- Enter 키 / 검색 버튼 클릭 → 현재 입력값으로 이동
- 검색 결과는 `StorePage` (`/product/list`)에서 `?keyword=` 파라미터를 `useStorePageController`가 읽어 처리
...
const stateRef = useRef({}) // document keydown 핸들러의 stale closure 방지
